// ============================================================
// Car Racing — canvas game from the car_racing_test folder
// (built during the workshop with 5 Cline prompts, see
// car_racing_test/prompts.md).
//
// Embedded version: the loop and the keyboard listeners only run
// while the Car Racing page is active, so the game never
// interferes with the rest of the site (flashcards, nav, ...).
// ============================================================
...
(() => {
  ...
  function update() {
    ...
    // Simple AABB collision detection between player and each enemy
  ...
  function drawRoad() {
    ...
    // Dashed white center line that scrolls downward
  ...
  // Show a "Game Over" message with the final score
  ...
  function gameLoop() {
    // When gameOver is true, stop the game loop
    if (gameOver) {
      drawGameOver();
  ...
  // ===== Embedding: start/stop the game with the page =====
  const racingPanel = document.getElementById('carracing');
  let isActive = Boolean(racingPanel && racingPanel.classList.contains('active'));
  ...
  document.addEventListener('pagechange', (event) => {
    const active = event.detail.page === 'carracing';
  ...
  // Keyboard: left/right to steer, Space to restart after a crash.
  // Ignored while another page is open or while a button/link/input is
  // focused, so site navigation and the flashcards keep working.
  document.addEventListener('keydown', (event) => {
    if (!isActive) return;
    const el = document.activeElement;
    if (el && (el.tagName === 'BUTTON' || el.tagName === 'A' || el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.tagName === 'SELECT' || el.isContentEditable)) return;
    ...
    // Restart the game when the player presses the Spacebar
  ...
  // Draw one static frame so the canvas is never blank inside the panel.
  resetGame();
  draw();
})();